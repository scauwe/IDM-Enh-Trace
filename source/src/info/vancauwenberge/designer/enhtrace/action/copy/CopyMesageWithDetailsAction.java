...
public class CopyMesageWithDetailsAction extends Action {
	private final ILogMessage message;

	public CopyMesageWithDetailsAction(String menuLabel, ILogMessage message) {
		super(menuLabel);
	...
	public void run(){
		...
		StringBuilder text = new StringBuilder(message.getMessage());
		text.append('\n');
		text.append(message.getMessageDetail());
		final Clipboard cb = new Clipboard(PlatformUI.getWorkbench().getDisplay());
		cb.setContents(new Object[] { text.toString() }, new Transfer[] { textTransfer });
	}
}